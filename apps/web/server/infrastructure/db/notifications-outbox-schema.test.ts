...
describeWithPostgres('notification and outbox authority schema', () => {
  ...
  it('commits a domain event and in-app notification atomically', async () => {
    ...
    try {
      ...
      const persisted = await database.executor.query<{ event_count: string, notification_count: string }>(
        `SELECT
  ...
  it('rolls back the event when notification creation fails', async () => {
    ...
    const persisted = await database.executor.query<{ count: string }>(
  ...
  it('deduplicates repeated mail consumer delivery creation', async () => {
    ...
    const consume = () => database.executor.query(
    ...
    const deliveries = await database.executor.query<{ count: string }>(
