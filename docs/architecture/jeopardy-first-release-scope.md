...
此清单用于代码审查与 `pnpm check:jeopardy-scope` 自动检查。首期交付只有 Jeopardy；未列入的未来能力不能以隐藏路由、兼容枚举、空数据表或未调用 Worker operation 的形式预埋。

## 首期纳入

- Nuxt/Nitro 唯一公网控制面、PostgreSQL 权威事实、Redis 派生缓存、S3 兼容内容存储；
- 密码登录、sealed Cookie、邮箱验证与找回、全局角色、默认管理员治理；
- 队伍、邀请、报名审核、分组和成员锁定；
- Jeopardy 比赛生命周期、公告、公开时间线、封榜和赛后练习；
- 题库不可变版本、比赛题目快照、同步 Flag 校验、固定与衰减计分；
- 排行榜、SSE、Writeup、比赛包、通知、审计和运维命令；
...
`pnpm check:jeopardy-scope` 在以下首期产物中检查禁入标识：

- `api/openapi.yaml` 的路径、Schema 与枚举；
