...
首页和 `/login` 页面现在会先读取 `/api/auth/setup-status`，只有在默认管理员仍然有效时才显示这对账号。
`/login` 表单也只会在这种空库场景下预填 `admin / sauryctf`，避免已有用户库仍误导去尝试默认口令。
...
- `注册`
- 首页现在也补了一层首次使用引导：
  - 管理员入口会明确提示 `admin / sauryctf` 仅在空库首次启动时可用
  - 普通选手入口会先引导去注册，若来自比赛页，注册成功后会先进入队伍页再回到原比赛
  - 公开比赛入口会直接跳到 `/games`，便于先浏览公开信息和榜单
  - 首页顶部还会根据“已登录 / 空库管理员阶段 / 普通访客”切换一条明确的“当前下一步”提示
...
如果你是第一次在本地把整套项目跑起来，建议先直接检查这组默认入口：
...
如果你想更快完成首次配置，管理端现在也提供了两个现成模板：

- 创建比赛时可直接点“填充基础模板”，快速生成一场适合首次配置的公开比赛
- 创建题目时可直接点“基础题”，快速补齐题目结构；正式发布前仍需自行填写题面、入口和 Flag
