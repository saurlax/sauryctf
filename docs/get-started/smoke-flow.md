...
4. 创建一题动态题并挂题
5. 导出这场比赛的比赛包
6. 将刚导出的比赛包重新导入，并确认导入后的比赛回到 `draft`
7. 检查导入后的比赛仍然保留原来的两道挂题
8. 激活原比赛
9. 注册普通用户
10. 创建队伍
11. 报名比赛
12. 检查动态题实例初始 `idle` 状态
13. 启动动态题实例租约，并确认预设入口已解析成当前队伍专属地址
14. 确认实例响应里已经带回当前 lease policy，并且刚启动时还不能立刻续期
15. 提交静态题正确 Flag
16. 以游客身份检查公开榜单

为了减少 Windows PowerShell 编码差异带来的误判，这条脚本当前不会再强依赖某一段固定的中文续期提示文案，而是优先校验 `policy`、`can_renew` 和入口字段是否正确返回。

当前自动检查还会顺手覆盖一条最小运维闭环：

- 比赛导出 ZIP 能正常下载
- 导入同一份 ZIP 后会创建新的 `draft` 比赛
- 导入后的比赛仍保留原有挂题关系
