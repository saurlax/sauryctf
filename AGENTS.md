...
- In `/console/admin`, contest announcement maintenance should use one shared `UModal` flow for create/edit work where possible; the page body should remain a compact timeline/list instead of keeping a second inline editor.
- In `/console/admin`, entry buttons that lead to existing announcement create/edit flows should call the shared modal-opening helpers instead of mutating modal state inline, so draft reset and route-context handling stay consistent.
...
- In `/console/admin` monitoring views, when a card, timeline item, lease row, or clue row already knows its `challenge_id`, prefer carrying that context through the jump so submissions, clues, and instance rows can highlight the same challenge instead of only jumping to the broader section.
